...
function linx_related_posts() {
	...
	if ( $tags && linx_get_option( 'linx_disable_related_posts', false ) == false ) {
		$tag_ids = array();
		foreach ( $tags as $tag ) {
			$tag_ids[] = $tag->term_id;
		}
...
function linx_show_hero() {
	return
		...
		( is_singular( 'post' ) || is_page() ) && linx_compare_options( linx_get_option( 'linx_hero_single_style', 'none' ), rwmb_meta( 'linx_hero_single_style' ) ) != 'none';
}
...
function linx_show_category_boxes() {
	return is_front_page() && linx_get_option( 'linx_disable_category_boxes', false ) == false && ! empty( linx_get_option( 'linx_box_categories', '' ) );
}
...
function linx_social_links() {
    ...
    array( 'name' => 'Tumblr', 'option' => 'tumblr_url', 'icon' => 'tumblr', 'color' => '#35465c' ),
    ...
    array( 'name' => 'Vine', 'option' => 'vine_url', 'icon' => 'vine', 'color' => '#00b488' ),
    array( 'name' => 'VK', 'option' => 'vk_url', 'icon' => 'vk', 'color' => '#45668e' ),
