...
jQuery(window).scroll(function() {
  ...
  if (body.hasClass('navbar-sticky_transparent')) {
...
function navbar() {
  ...
  var element = jQuery('.navbar-sticky_transparent.with-hero, .navbar-sticky_transparent.with-featured-wrapper');

  if (st > 80) {
    element.addClass('navbar-translucent');
  } else {
    element.removeClass('navbar-translucent');
  }
}
...
function hero() {
  ...
  if (body.hasClass('with-hero')) {
    if (body.hasClass('navbar-regular')) {
      jQuery('.hero-full .hero').height(jQuery(window).height() - jQuery('.site-header').height() - jQuery('#wpadminbar').height());
    } else {
      jQuery('.hero-full .hero').height(jQuery(window).height() - parseInt(body.css('padding-top')) - jQuery('#wpadminbar').height());
    }
...
function offCanvas() {
  ...
  jQuery('.widget-area').clone().appendTo(offCanvas);
