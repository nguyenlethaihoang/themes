		...
		<div class="ovsb-css-mgmt__left">
			...
			<template x-for="sheet in getSheetsButNotFolders()">
					...
					<li 
						...
						@click.stop="selectedSheet = sheet.id; sheet.css ? codeMirrorEditor.getDoc().setValue( b64_to_utf8(sheet.css) ) : codeMirrorEditor.getDoc().setValue( '' )">
							<span x-show="sheet.parent" x-text="getFolderName(sheet.parent) + '&nbsp/&nbsp'">--</span>
							<a 
								x-text="sheet.name" 
								href="#"
								class="ovsb-css-mgmt__sheet"
								@click.stop.prevent="selectedSheet = sheet.id; sheet.css ? codeMirrorEditor.getDoc().setValue( b64_to_utf8(sheet.css) ) : codeMirrorEditor.getDoc().setValue( '' )"></a>
		...
		:class="fullScreen && 'ovsb-css-mgmt__right--fullscreen'">
			<div class="ovsb-css-mgmt__controlbar">
				...
				<div 
					...
					<div class="ovsb-css-mgmt__theme-chooser">
						<label for="">Theme </label>
						<select id="ovsb-css-mgmt__themechooser" 
							x-on:change="updateCodeMirrorTheme()"
							x-model="currentTheme">
							<option value="default"><?php _e("Default","oxygen") ?></option>
							<option value="dracula"><?php _e("Dracula","oxygen") ?></option>
							<option value="midnight"><?php _e("Midnight","oxygen") ?></option>
							<option value="eclipse "><?php _e("Eclipse ","oxygen") ?></option>
						</select>
					</div>
					<button 
			...
			<textarea id="#ovsb-css-codemirror"></textarea>
		</div>
	</div>
	<?php
}
